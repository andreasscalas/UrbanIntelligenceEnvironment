...
#include "ui_annotationrelationshipdialog.h"

using namespace SemantisedTriangleMesh;
AnnotationsRelationshipDialog::AnnotationsRelationshipDialog(QWidget *parent) : QDialog(parent), ui(new Ui::AnnotationsRelationshipDialog){
    ui->setupUi(this);
...
AnnotationsRelationshipDialog::~AnnotationsRelationshipDialog(){
    delete ui;
}

void AnnotationsRelationshipDialog::slotType(QString type)
{
    ...
    if(this->type.compare("") == 0) {
        ...
        this->ui->measure1ComboBox->setEnabled(false);
        this->ui->measure2ComboBox->setEnabled(false);
    } else if(this->type.compare("Adjacency") == 0) {
        this->directed = false;
        this->ui->doubleSpinBox1->setValue(1.0);
        this->ui->doubleSpinBox1->setEnabled(false);
        this->ui->doubleSpinBox2->setValue(1.0);
...
void AnnotationsRelationshipDialog::slotSave(){
    this->close();
...
std::string AnnotationsRelationshipDialog::getTypeString() const
{
    return type;
}

std::vector<std::shared_ptr<Annotation> > AnnotationsRelationshipDialog::getSubjects() const
{
    return subjects;
}

void AnnotationsRelationshipDialog::setSubjects(const std::vector<std::shared_ptr<Annotation> > &selectedAnnotations)
{
    subjects = selectedAnnotations;
    this->ui->typeList->clear();
    this->ui->measure1ComboBox->clear();
    this->ui->measure2ComboBox->clear();
    if(selectedAnnotations.size() > 0){
        //AnnotationType currentType = selectedAnnotations[0]->getType();
        if(selectedAnnotations.size() == 2)
        {
            this->ui->typeList->addItem("Adjacency");
        }
...
//            }
//            for(unsigned int i = 0; i < selectedAnnotations[0]->getAttributes().size(); i++)
//            {
//                std::string s = selectedAnnotations[0]->getAttributes()[i]->getKey() + " with id " + std::to_string(selectedAnnotations[0]->getAttributes()[i]->getId());
...
//                if(currentType != selectedAnnotations[i]->getType())
...
//                for(unsigned int i = 0; i < selectedAnnotations[1]->getAttributes().size(); i++)
//                {
//                    std::string s = "\""+selectedAnnotations[1]->getAttributes()[i]->getKey() + "\"" + " with id " + std::to_string(selectedAnnotations[1]->getAttributes()[i]->getId());
//                    this->ui->measure2ComboBox->addItem(QString::fromStdString(s));
//                }
//            }
//        }
